...
Covers:
  ...
  Negative tests (receive_file) — each asserts:
...
def _transfer(
    ...
    """Helper: send src_path from sender → receiver; write result to dest_path."""
    buf = io.BytesIO()
...
class TestNonceCache:
    ...
    def test_different_nonce_accepted(self):
        ...
        cache.check_and_insert(kid, b"\x02" * 16, ttl_seconds=60)  # different nonce — ok

    def test_different_sender_accepted(self):
        cache = NonceCache()
        nonce = b"\x01" * 16
        cache.check_and_insert(b"\xAA" * 8, nonce, ttl_seconds=60)
        cache.check_and_insert(b"\xBB" * 8, nonce, ttl_seconds=60)  # same nonce, different sender — ok
    ...
    def test_many_inserts_evict_correctly(self, monkeypatch):
        ...
        # Cache should not grow unboundedly — entries older than TTL are gone
    ...
    def test_concurrent_threading_exactly_one_wins(self):
        ...
        a broken version with ``time.sleep(0)`` between check and insert —
        ...
        - Safety under GIL-free Python (CPython 3.13 free-threaded build);
          ...
          specifically.
        - Asyncio safety — ``check_and_insert`` is synchronous and the lock
...
class TestRoundtrip:
    ...
    def test_exact_chunk_boundary(self, tmp, alice, bob, bob_keystore):
        """File size exactly equal to CHUNK_SIZE — tests edge case in read-ahead."""
    ...
    def test_filename_preserved_in_metadata(self, tmp, alice, bob, bob_keystore):
        ...
        tampering would cause AuthenticationError — round-trip confirms encoding)."""
...
class TestNegative:
    ...
    def test_wrong_receiver_key_raises(self, tmp, alice, bob, bob_keystore):
        """A receiver with the wrong private key cannot decrypt — AuthenticationError."""
        src = tmp / "secret.bin"
        ...
        with pytest.raises(AuthenticationError):
            receive_file(wrong_identity, bob_keystore, buf, dest, NonceCache())

        _assert_no_partial(dest)

    def test_tampered_data_chunk_raises(self, tmp, alice, bob, bob_keystore):
        """Flipping a bit in a data chunk → AuthenticationError; .part deleted.
        ...
        at byte 124. Offset 257 is byte +133 inside chunk 1's ciphertext —
        ...
        with pytest.raises(AuthenticationError):
            receive_file(bob, bob_keystore, io.BytesIO(bytes(raw)), dest, NonceCache())

        _assert_no_partial(dest)

    def test_truncated_stream_raises(self, tmp, alice, bob, bob_keystore):
        """Dropping the final chunk → TruncationError or AuthenticationError; .part deleted."""
        src = tmp / "data.bin"
        src.write_bytes(os.urandom(200_000))   # multiple chunks
        dest = tmp / "out.bin"
        buf = self._send_to_buffer(alice, bob, src)

        # Truncate to 60% of stream — removes final chunk(s)
        raw = buf.getvalue()
        truncated = raw[: int(len(raw) * 0.6)]

        with pytest.raises((TruncationError, AuthenticationError, ProtocolError)):
            receive_file(bob, bob_keystore, io.BytesIO(truncated), dest, NonceCache())

        _assert_no_partial(dest)

    def test_replay_within_ttl_raises(self, tmp, alice, bob, bob_keystore):
        """Replaying an identical transfer within the TTL window → ReplayError."""
        src = tmp / "data.bin"
        ...
        cache = NonceCache()
        receive_file(bob, bob_keystore, io.BytesIO(raw), dest1, cache)

        with pytest.raises(ReplayError):
            receive_file(bob, bob_keystore, io.BytesIO(raw), dest2, cache)

        # ReplayError is raised before .part is opened — both checks apply
        _assert_no_partial(dest2)

    def test_expired_transfer_raises(self, tmp, alice, bob, bob_keystore):
        """Transfer with ttl=1 received after 2 seconds → TTLError."""
        src = tmp / "data.bin"
        ...
        try:
            ...
            with pytest.raises(TTLError):
                receive_file(bob, bob_keystore, io.BytesIO(raw), dest, NonceCache())
        finally:
    ...
    def test_unknown_sender_raises(self, tmp, alice, bob):
        """Sender's key_id not in keystore → UnknownSenderError."""
        src = tmp / "data.bin"
    ...
    def test_tampered_signature_raises(self, tmp, alice, bob, bob_keystore):
        """Ed25519 signature tampered → SignatureError; .part file deleted.

        AEAD protects the metadata chunk (including the signature field), so
        an attacker cannot tamper with the sig byte directly — the AEAD tag
        ...
        with pytest.raises(SignatureError):
            receive_file(bob, bob_keystore, forged_buf, dest, NonceCache())
