...
Covers:
  - derive_stream_key: same inputs → same output; different stream_header → different key
  - SecretstreamPusher/Puller: happy path round-trip
  - Negative: tampered ciphertext → AuthenticationError
  - Negative: out-of-order chunks → AuthenticationError
  - Negative: chunk from different stream (wrong key) → AuthenticationError
  - Negative: push after push_final → RuntimeError
  - sign_digest / verify_digest: happy path
  - Negative: wrong pubkey → SignatureError
  - Negative: tampered digest → SignatureError
  - Negative: tampered signature → SignatureError
...
class TestSecretstreamNegative:
    ...
    def test_out_of_order_chunks_raises(self):
        ...
        puller = SecretstreamPuller(stream_key, pusher.header)
        # Feed chunk 1 before chunk 0 — stream state machine rejects it
        with pytest.raises(AuthenticationError):
    ...
    def test_injected_chunk_from_other_stream_raises(self):
        ...
        # Stream B — same key, different stream (different header)
...
class TestSignVerify:
    ...
    def test_deterministic(self):
        """Ed25519 signatures are deterministic — same inputs, same output."""
        seed, _ = _ed25519_keypair()
        digest = os.urandom(32)
